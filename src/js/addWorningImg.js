export const addWorningImg = (id = '') => {
  const errorBlock = document.getElementById(`error_name`);
  let textError = '';
  let svgBlokc = document.getElementsByClassName('svgWornin');

  if (errorBlock) {
    textError = errorBlock.textContent;
  }
  ...
  if (textError && svgBlokc) {
    console.log('return');
    return;
  }

  if (textError /*&& !svgBlokc*/) {
    console.log(svgBlokc);

    svgBlokc = document.createElement('div');

    svgBlokc.classList.add('svgWorning');

    errorBlock.append(svgBlokc);

    svgBlokc.innerHTML = svg;
  }
};
